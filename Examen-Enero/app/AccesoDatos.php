...
class AccesoDatos {
   ...
   // Constructor privado  Patron singleton
    ...
    private function __construct(){
        ...
        $this->stmt_incrementar = $this->dbh->prepare("update Usuarios set saldo = saldo * 1.10 where login=:login");
        $this->stmt_bloqueotodo = $this->dbh->prepare("update Usuarios set bloqueo = 0");
        $this->stmt_desbloqueo = $this->dbh->prepare("update Usuarios set bloqueo = 1 where login=:login");
    ...
    //INCREMENTO
    public function incrementarUsuario($login){
        $this->stmt_incrementar->bindValue(':login', $login);
        if ($this->stmt_incrementar->execute()) {
            return true;
        }else return false;
    }

    //BLOQUEO TODO
    public function bloquearTodo(){
        if ($this->stmt_bloqueotodo->execute()) {
            return true;
        }else return false;
    }

    //DESBLOQUEO
    public function desbloquearUsuario($login){
        $this->stmt_desbloqueo->bindValue(':login', $login);
        if ($this->stmt_desbloqueo->execute()) {
            return true;
        }else return false;
    }
